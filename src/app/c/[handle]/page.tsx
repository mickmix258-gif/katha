    ...
    <div className="mx-auto max-w-3xl px-4 py-10">
      ...
      <div className="mt-3 grid gap-2">
        {characters
          ...
          .map((item) => (
            <Link key={item.id} href={`/characters/${item.id}`} className="rounded-xl border border-[var(--line)] p-3">
              ตัวละคร · {item.name}
            </Link>
          ))}
        {scenes
          .filter((item) => item.creatorHandle === handle)
          .map((item) => (
            <Link key={item.id} href={`/scenes/${item.id}`} className="rounded-xl border border-[var(--line)] p-3">
        ...
        {worlds
          .filter((item) => item.creatorHandle === handle)
          .map((item) => (
            <Link key={item.id} href={`/worlds/${item.id}`} className="rounded-xl border border-[var(--line)] p-3">
              โลก · {item.title}
            </Link>
          ))}
      </div>
